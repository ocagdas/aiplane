...
class IntegrationChatTests(unittest.TestCase):
    ...
    def test_integrations_plan_selects_defaults_best_and_manual_overrides(self) -> None:
        with _isolated_test_profile() as profile:
            ...
            self.assertEqual(
                default_plan["selection"]["chat"]["name"], "fixture-chat-small"
            )
            self.assertIn(
                "tool_use", default_plan["selection"]["chat"]["role_capabilities"]
            )
            self.assertEqual(
                default_plan["selection"]["autocomplete"]["name"], "fixture-code-base"
            )
            self.assertEqual(
            ...
            self.assertTrue(
                all(
                    row["runtime"] == "ollama"
                    for row in best_plan["selection"].values()
                )
            )
    ...
    def test_integrations_setup_dry_run_plans_runtime_actions(self) -> None:
        ...
        self.assertTrue(
            all(action["status"] in {"planned", "ok"} for action in result["actions"])
        )
    ...
    def test_integrations_setup_pulls_huggingface_gguf_alias_through_ollama(
        ...
        with (
            ...
            patch.object(
                IntegrationManager, "_run_with_progress", return_value=completed
        ...
        pull_actions = [
            action for action in result["actions"] if action["action"] == "pull"
        ]
    ...
    def test_integrations_setup_passes_profiles_dir_to_helper(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch.object(
                IntegrationManager, "_run_with_progress", side_effect=fake_run
            ):
                action = IntegrationManager(profile)._setup_action(
                    "ollama",
                    "pull",
        ...
        self.assertEqual(action["status"], "succeeded")
        self.assertEqual(
            captured["env"]["AIPLANE_PROFILES_DIR"], str(profile_root.parent)
        )
    ...
    def test_integrations_setup_failure_includes_sanitized_output_tail(self) -> None:
        ...
        self.assertEqual(
            action["stderr_tail"], ["pulling manifest", "error: not found"]
        )
    ...
    def test_integrations_plan_and_setup_cli(self) -> None:
        with _isolated_profiles_dir() as profiles_dir:
            ...
            with redirect_stdout(stdout):
                code = cli_main(
                    profiles_arg + ["integrations", "setup", "continue", "--dry-run"]
                )
            self.assertEqual(code, 0)
    ...
    def test_integrations_export_continue_uses_planner_constraints(self) -> None:
        with _isolated_test_profile() as profile:
            ...
            self.assertIn(
                f"model: {planned['selection']['chat']['model']}", exported.content
            )
            self.assertIn(
            ...
            self.assertIn(
                f"model: {planned['selection']['embedding']['model']}", exported.content
            )
            self.assertIn("apiBase: http://localhost:11434/v1", exported.content)
    ...
    def test_integrations_continue_single_model_export_still_works(self) -> None:
        ...
        exported = IntegrationManager(profile).export(
            "continue", "fixture-analysis-small"
        )
        self.assertEqual(exported.tool, "continue")
        self.assertIn("apiBase: http://localhost:11434/v1", exported.content)
    ...
    def test_agents_plan_and_export_cli_print_scaffold(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(
                payload["selection"]["model_alias"], "fixture-analysis-small"
            )
    ...
    def test_agent_artifacts_root_uses_env_config_and_cli_override(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            config_path.write_text(
                f"agent_artifacts_dir: {configured}\n", encoding="utf-8"
            )
    ...
    def test_integrations_export_uses_named_credential_ref(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            try:
                profile = load_profile("local-dev", Path.cwd())
                profile.models["providers"]["openai"][
                    "credential_ref"
                ] = "openai.personal"
                profile.models["models"]["managed-chat-small"]["enabled"] = True
                exported = IntegrationManager(profile).export(
                    "continue", "managed-chat-small"
                )
    ...
    def test_integrations_plan_supports_single_model_targets(self) -> None:
        ...
        self.assertEqual(
            payload["selection"]["primary"]["name"], "fixture-analysis-small"
        )
        self.assertEqual(
            payload["selection"]["primary"]["endpoint"], "http://localhost:11434/v1"
        )
    ...
    def test_integrations_export_mcp_client_configs(self) -> None:
        profile = load_profile("local-dev", Path.cwd())
        vscode = IntegrationManager(profile).export(
            "vscode-mcp", "fixture-analysis-small"
        )
        continue_mcp = IntegrationManager(profile).export(
            "continue-mcp", "fixture-analysis-small"
        )
        generic = IntegrationManager(profile).export(
            "generic-mcp", "fixture-analysis-small"
        )
    ...
    def test_chat_endpoint_dry_run_previews_runtime_agnostic_plan(self) -> None:
        profile = load_profile("local-dev", Path.cwd())
        output = IntegrationManager(profile).run_chat(
            None, prompt="hello", dry_run=True
        )
    ...
    def test_chat_native_ollama_opt_in_resolves_huggingface_gguf_for_ollama(
        ...
        command = IntegrationManager(profile).run_chat(
            "hf-gguf-chat", dry_run=True, native_ollama=True
        )
    ...
    def test_chat_rejects_non_chat_capable_model(self) -> None:
        ...
        with self.assertRaisesRegex(ValueError, "not suitable for chat execution"):
            IntegrationManager(profile).run_chat(
                "fixture-embedding-small", prompt="hello", dry_run=True
            )
    ...
    def test_managed_provider_alias_exports_continue_config(self) -> None:
        ...
        with redirect_stdout(stdout):
            code = cli_main(
                ["integrations", "export", "continue", "--model", "managed-chat-small"]
            )
        self.assertEqual(code, 0)
        output = stdout.getvalue()
