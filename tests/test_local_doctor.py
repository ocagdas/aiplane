...
class LocalDoctorTests(unittest.TestCase):
    def test_local_coding_doctor_reports_wedge_sections(self) -> None:
        ...
        defaults = next(
            ...
            if section["name"] == "model_defaults"
        )
        ...
        mcp = next(
            section for section in payload["sections"] if section["name"] == "mcp"
        )
        ...
        self.assertEqual(
            mcp_checks["mcp_local_coding_read_surface"]["missing_tools"], []
        )
        required_tools = set(
            mcp_checks["mcp_local_coding_read_surface"]["required_tools"]
        )
        ...
        self.assertIn(
            "aiplane.models.use", mcp_checks["mcp_guarded_write_surface"]["tools"]
        )
        self.assertTrue(
            any("mcp manifest" in step.lower() for step in payload["next_steps"])
        )
    ...
    def test_doctor_reports_default_endpoint_and_hardware_fit_details(self) -> None:
        ...
        chat_check = next(
            check for check in defaults["checks"] if check["name"] == "chat_model"
        )
        ...
        endpoints = next(
            section for section in payload["sections"] if section["name"] == "endpoints"
        )
        self.assertTrue(
            any(check["name"] == "endpoint:chat" for check in endpoints["checks"])
        )

        hardware = next(
            section for section in payload["sections"] if section["name"] == "hardware"
        )
        self.assertTrue(
            any(check["name"] == "active_machine" for check in hardware["checks"])
        )
    ...
    def test_doctor_reports_reachable_endpoint_for_openai_compatible_default(
        ...
        with TestHttpServer() as endpoint:
            ...
            profile.models.setdefault("defaults", {})[
                "chat_model"
        ...
        chat_endpoint = next(
            check for check in endpoints["checks"] if check["name"] == "endpoint:chat"
        )
    ...
    def test_doctor_rejects_incompatible_default_alias_for_integration_role(
        ...
        integrations = next(
            section
            for section in payload["sections"]
            if section["name"] == "integrations"
        )
        continue_check = next(
            check
            for check in integrations["checks"]
            if check["name"] == "integration:continue"
        )
