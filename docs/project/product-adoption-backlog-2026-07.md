...
2. **Publish the first complete developer-preview release artifact — publication workflow hardened; complete public assets pending.** CI owns patch commits and annotated artifact tags, rejects PR version edits and preserves authorized direct-main minor/major versions, validates the exact tagged wheel, and manual release dispatch now refuses incomplete wheel/source/checksum sets and verifies attachments after publication. Signing is required only with a maintainable trusted identity. Completion: a public immutable release URL contains exactly one wheel, one source distribution, `SHA256SUMS`, supported-platform/upgrade/rollback guidance, and metadata matching its tag.
3. **Verify the public no-clone path — cross-platform workflow implemented; successful public run pending.** `Verify published release` downloads the actual named release on Linux, macOS, and Windows, verifies the portable manifest, exercises pip/pipx/uv independently, and uploads canonical sanitized evidence. Final evidence still requires a complete P0.2 release and a successful hosted run. Completion: the nine evidence records identify release URL, checksum, tag commit, commands, results, and platform limitations.
4. **Make CI mandatory on the release path — repository implementation and exact settings guide complete; hosted activation pending.** `CI / Release gate` aggregates quality, compatibility, and cross-OS install jobs; the guide covers PR/status requirements, force/deletion protection, the narrow Actions bypass required by CI-owned commits/tags, tag immutability, and read-only audit commands. Completion still requires an active hosted ruleset and a blocked-merge test.
