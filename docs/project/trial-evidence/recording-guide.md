...
Use the [Field Evidence Collection Runbook](../../user/evidence-collection.md) for the copy-paste capture procedure. Use one sanitized JSON record for every P0 workflow rehearsal and independent-user trial. Copy [`trial-evidence/template.json`](template.json), fill every field, and validate it before sharing:
