...
CI artifacts are retained for 30 days, require repository/workflow access, and are not immutable public releases. They must not be advertised as the P0 release URL. Intentional minor/major versions publish a durable GitHub Release automatically; patch versions remain artifacts unless a maintainer selects the tag through manual release dispatch. Every published release rebuilds its wheel and source distribution from the immutable tag.
