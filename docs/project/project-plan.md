...
### Latest Working-Tree Handoff

The current uncommitted P0 maintenance slice makes the CI-used `make check` gate strict: `make format` remains the explicit developer autofix command, while new `make format-check` fails on drift before lint and isolated-profile tests run. `CONTRIBUTING.md` now uses the current environment-doctor/configuration-compiler description and names maintained focused suites rather than the removed `test_mvp.py`. The roadmap records the remaining verified maintenance items for tool command semantics, fixture-root portability, defensive dispatch/governance contracts, and bounded ownership-driven module extraction. Validation: focused contracts pass 38 tests; `make check` passes formatting, lint, and the full isolated-profile suite; profile validation plus required text/JSON environment doctors pass.

The current uncommitted P1 maintenance slice aligns `ToolExecutor` defaults with the project’s development gates: `run_tests` uses pytest, `lint` uses Ruff, `build` remains a compile check, and nonzero commands fail the tool action with their diagnostic instead of reporting success. Shared test bootstrap and profile fixtures now resolve repository templates from their own file location, so fixture setup survives a temporary working directory. The roadmap marks both verified P1 items complete and keeps defensive dispatch/governance and ownership-led module work in P2. Validation: focused boundary, fixture-portability, and environment-tool suites pass 30 tests; the full suite passes 834 tests with 9 opt-in checks skipped; format, lint, and diff hygiene pass.

The current uncommitted P2 maintenance slice replaces the CLI’s bare unhandled-command fall-through with an actionable parser error, preserves critical documentation/safety contracts while making roadmap numbering continuous rather than endpoint-fixed, and changes module-size limits to visible extraction warnings. Profile-governed `ToolExecutor` execution now has its own `tool_execution.py` ownership boundary; toolchain discovery and environment-doctor work remain in `tools.py`. Validation: focused architecture, CLI-error, and environment-tool suites pass 37 tests; the full suite passes 836 tests with 9 opt-in checks skipped; format, lint, and diff hygiene pass.

The current uncommitted P3 maintenance slice completes the remaining low-risk findings: a child-process profile-validation check runs from an external working directory with only the repository source explicitly available, proving the test/profile setup is not tied to the caller’s current directory; and the tracked assistant skill plus its manifest now use the same environment-doctor/configuration-compiler boundary as public package metadata. The skill no longer names the removed `test_mvp.py`; contract coverage prevents both regressions. Validation: focused portability, contracts, and security-documentation suites pass 42 tests; strict `make check`, profile validation, required text/JSON environment doctors, and diff hygiene pass.

The current uncommitted P2.26 policy-consistency slice closes an integration boundary: plan, setup, direct export, and saved-plan export now apply the selected alias’s repository provider/cloud policy before producing configuration. This prevents a policy-blocked managed endpoint from being emitted through CLI or MCP delegation without treating discovered/manual aliases as profile-owned models or leaking action-scoped grants. User guidance explains the read-only failure and remedy. Validation: focused policy, integration, and MCP suites pass 96 tests plus 13 subtests.

The current uncommitted agent-framework usability group is implementation-complete and ready for human review/commit. All seven agent targets now render `endpoint-smoke.py` plus `endpoint-smoke-requirements.txt` containing only `openai`, so an endpoint check does not require a framework install. `agents export` chooses that smoke runner by default for non-native targets; LangGraph and `simple-openai` retain their native `agent.py` starters. Framework metadata remains render-only and is never represented as a complete native project for CrewAI, AutoGen, Semantic Kernel, LlamaIndex Workflows, or OpenHands.

Framework readiness now validates the local Python minimum, HTTP(S) endpoint shape, expected OpenAI-compatible protocol, and declared model chat role. It reports the locally observed framework package version or a missing-package warning without importing or installing a framework. It does not contact an endpoint, write credentials, install packages, queue a job, or run an autonomous agent. User docs, manual copy/paste checks, command help, roadmap entries, and the current command-coverage table reflect the new starter contract.

Validation for this working-tree group: agent artifact/schema/benchmark coverage passes 58 tests; adjacent integration, stack, command-contract, and security-documentation coverage passes 102 tests plus 19 subtests; the generated endpoint-smoke script compiles; repository-wide Ruff format/lint and `git diff --check` pass. The changed-line secret scan found only documentation and environment-variable references, never credential values. Run `python -m pytest` again as part of the pre-commit/release gate if a fresh full-suite record is needed.

The current uncommitted follow-up extends `agents doctor`: it observes installed distribution metadata without imports, supports an opt-in credential-free endpoint inventory probe for direct or stack-derived roles, and can perform one explicitly requested fixed chat-completions probe after inventory confirms the selected model. The chat probe sends no credentials, does not expose generated text, and never starts an agent. It does not establish framework-version semantic compatibility; that remains an evidence task requiring externally installed framework and endpoint validation, still opt-in, secret-free, and outside Aiplane's agent-execution boundary. The manual validation guide now starts with a concise grouped A–F path for installation/profile isolation, host and catalog discovery, local runtime/chat configuration, host-client exports, agent-environment evidence, and reproducibility/stacks, before the exhaustive acceptance checklist. Validation for the current diff: focused security-documentation, agent, and boundary tests pass 41 tests; the full suite passes 831 tests with 9 opt-in checks skipped; formatting, lint, and diff hygiene pass.
...
7. Completed: public onboarding has top-level `discover`, `doctor`, `pick`, `recommend`, and `export` commands with help text and tests.
...
Runtime compatibility, workflow readiness, and bundle contract hardening are implementation-complete at the deterministic boundary. Stack setup/export rejects incompatible model/runtime pairs; primary runner endpoint, port, protocol, substrate, health, cache, and bundle-mode facts share one typed contract; local-runtime readiness requires an actually available platform-compatible runner; required-only doctors skip optional probes; bundle options are mode-aware; llama.cpp is native-only until a runnable container exists; Docker image names are safe; and recipe, version, and digest evidence levels are explicit.
...
- Provider, runtime-packaging, and agent-environment hardening is complete at the reviewed contract boundary. Offline provider diagnostics, Anthropic model discovery, optional-auth local OpenAI-compatible discovery, schema-linked runtime bundles, and profile/stack agent manifests plus CLI-only stack-bound job/handoff artifacts are available; MCP remains limited to the safe environment-manifest read surface. Focused acceptance passes 156 tests plus 13 subtests; the full serial suite passes 611 tests with 3 optional tests skipped and 23 subtests. Ruff format/lint, profile validation, and required-only environment doctor checks in text and JSON pass. The parallel check wrapper reached format/lint but could not start pytest because this environment lacks the optional pytest-xdist plugin; the complete serial suite is authoritative for this run. The `jsonschema` development dependency is now pinned, all packaged schemas receive Draft 2020-12 meta-schema validation, and representative deployment/runtime payloads are validated against their public schemas. The documented loopback sandbox failure required narrow asserted rewrites with immediate diff verification.
...
The repeatable benchmark and runtime-evidence milestone is implementation-complete. Versioned JSON/YAML suites, deterministic validation, repeat statistics and uncertainty, safe built-in graders, explicitly opted-in trusted command graders, preview-first external measurement import, data-only scoring extensions, artifact locks, render-only runtime launch manifests, and evidence-backed role comparisons are available. Pull previews, benchmark records, stack plan/doctor, replay comparison, and stack lifecycle share those contracts. Ollama native duration/token/throughput fields and OpenAI-compatible usage counters populate benchmark evidence when exposed; benchmark-only Ollama streaming records client-observed TTFT with explicit provenance, while unavailable metrics remain null. Cross-machine calibration and complete artifact integrity still depend on external evidence rather than inference.
...
4. **Reviewed patch workflow - Implemented / guarded**
   - `aiplane patches inspect PATH` reads an already-reviewed user-supplied Git patch inside the selected workspace, reports a hash, affected files, bounded preview, and `git apply --check --verbose` result without changing files.
   - `aiplane patches apply PATH --yes` repeats validation and applies only when the existing `write_file` policy permits it; it audits the decision and never stages or commits changes.
   - The workflow never generates diffs, calls a model, resolves conflicts, or exposes arbitrary command execution. It rejects out-of-workspace input, unsafe target paths, oversized/empty input, and secret-like content before Git is invoked.
...
6. **Agent-to-agent setup and export - Implemented configuration foundation / live validation ongoing**
   - Implemented today: orchestrator catalog/readiness, package/setup scaffolding, stack binding, schema-linked secret-free agent manifests, versioned stack-bound job specifications, checksummed environment/job handoffs, and framework-specific topology/readiness starter exports.
   - Role bindings describe model aliases/native ids, managed or self-managed endpoints, tool policies, approval modes, limits, audit labels, and credential references without secret values. `control_enforcement` makes clear which requested controls are only validated/recorded and must be enforced by the target framework or wrapper.
   - Export starter configuration for established orchestrator frameworks, including an endpoint smoke path for each target, observed local package-version readiness, and opt-in credential-free endpoint inventory plus fixed chat-completions probes. Live framework-version semantic compatibility validation remains ongoing, and autonomous agent execution remains outside `aiplane`.
...
| OpenTofu / Terraform / Pulumi | Repeatable VM/AKS infrastructure scaffolds without reinventing IaC. | Render selected HCL or Python Pulumi families, expose official validation/preview commands, and keep apply external and guarded. | Render-only scaffolds implemented; production modules/live validation ongoing. |
...
21. **Completed implementation — defensive dispatch and durable governance checks.** Unhandled CLI dispatch now terminates with an actionable internal error naming the command. Documentation and release-safety contracts remain strict, while the roadmap-numbering check now requires contiguous numbering rather than a fixed endpoint and module-size thresholds warn when a cohesive extraction is due instead of blocking unrelated work.
