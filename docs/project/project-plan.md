...
Remaining agent-framework work is live framework-version compatibility and endpoint behavior validation against externally installed frameworks. That work must remain opt-in, secret-free, and outside Aiplane's agent-execution boundary.
...
| Agents | `agents templates/plan/manifest/export/job/handoff` plus `skills/aiplane/SKILL.md` | Versioned handoff, enforcement-status, and starter-readiness guidance implemented | Prints non-mutating starter files, a schema-linked agent-environment manifest, stack-bound job specifications, and checksummed environment/job handoffs for seven supported targets. Every target includes a direct OpenAI-compatible endpoint-smoke starter with a minimal independent requirement file; native `agent.py` remains limited to LangGraph and the simple OpenAI client. Readiness validates Python, endpoint/protocol, and declared chat role and reports observed framework package versions without importing them. `control_enforcement` explicitly separates Aiplane validation/recording from runtime enforcement for workspace, tools, approvals, limits, and audit labels. It preserves multi-role identities, endpoints, policy, approvals, limits, audit labels, credential references, task text, and workspace boundaries without secrets, package installation, job queuing, or agent execution. |
...
6. **Agent-to-agent setup and export - Implemented configuration foundation / live validation ongoing**
   ...
   - Export starter configuration for established orchestrator frameworks, including an endpoint smoke path for each target and observed local package-version readiness; live framework-version compatibility validation remains ongoing, and autonomous agent execution remains outside `aiplane`.
