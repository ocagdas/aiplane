...
| Experimental | `deploy list/show/workflow-plan/plan/doctor/apply --yes`, `orchestrators list/show/doctor/setup`, `stacks setup/list/show/plan/doctor/endpoint-plan/status/export/prepare/start/stop/restart`, `agents templates/plan/manifest/export`, `policy grant/revoke`, `patches inspect/apply --yes`, `run`, `tool [--yes] TOOL [ARGS...]`, `chat`, `bridge list/exec`, `launch`, `session start` | Commands may exist for experiments and advanced users, but they should not lead README, demos, or beta onboarding. They must not block the external beta workflow. |
