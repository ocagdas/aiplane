...
2. Next P1: evaluate narrow secret-free Continue/Aider import into an unapproved draft profile.
3. Next P1: publish provider/runtime/client support tiers, upstream versions, owners, and maintenance expectations.
4. Next P1: define a contributor adapter protocol with reusable fixtures and contract tests.
5. P2 after adoption evidence: prioritize Docker Model Runner parity across the six-runner capability matrix; then add a render-only Kubernetes artifact family (`ResourceClaim`, `Deployment`, `Service`, and Helm values); then implement benchmark-suite, artifact-lock, launch-manifest, repeated-quality, and role-routing contracts without growing service scope. Kubernetes application remains behind explicit review and approval, and a node scheduling REST service remains deferred until demonstrated user demand because CLI/MCP already cover most local planning workflows.
...
20. Prioritize Docker Model Runner parity across detection, inventory, alias/native-ID resolution, model fit, health, endpoint export, benchmarks, and guarded lifecycle. Once parity is evidence-backed, add deterministic render-only Kubernetes artifacts for `ResourceClaim`, `Deployment`, `Service`, and Helm values. Do not apply them implicitly: any application path requires a separately reviewed design, explicit human approval, and tested guardrails. Keep node scheduling REST deferred until user demand demonstrates that existing CLI/MCP planning is insufficient.
