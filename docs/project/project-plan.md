...
1. Must: run the clean-machine onboarding trial commands from `docs/project/project-plan.md#public-demo-plan` across the six Priority 9 environments and classify each failure.
...
5. Post-P0: turn the six-runner compatibility plan into an evidence-backed matrix, then close read/inventory/export gaps before considering new lifecycle mutations or agent-framework adapters.
6. Post-P0: implement the benchmark-suite, artifact-lock, launch-manifest, repeated-quality, and role-routing contracts in the Milestone 3 order; record REST/community-evidence decisions separately rather than growing service scope implicitly.
...
8. **Runtime packaging and deployment reproducibility** - Planned
   - Deliver runner coverage in staged tiers: Ollama and vLLM hardening; llama.cpp and LM Studio first-class inventory/endpoint/benchmark coverage; MLX support on Apple Silicon; and Docker Model Runner support through its official Docker Desktop contracts.
...
10. **Benchmark and recommendation quality** - Planned
   ...
   - Research, but do not pre-commit to, a node REST scheduling API or community benchmark exchange. CLI/MCP and local evidence remain the defaults unless user evidence and security/provenance gates justify another service surface.
