...
Post-P0 model-placement direction is now explicit in Roadmap Priority 14, Milestone 3, and P2 backlog item 19. Planned work separates hard compatibility, model-variant/resource estimates, measured runtime performance, task-quality evidence, and policy; adds quantization/context/KV-cache/execution-mode metadata, repeated TPS/TTFT/latency/token records, comparable local calibration, confidence/provenance, and near-miss explanations; and keeps all runtime execution delegated to external runtimes. This planning update does not change the current P0 command surface or demo scope.

Post-P0 runtime and agent-environment coverage is also explicit. The planned compatibility matrix covers Ollama, llama.cpp, MLX, Docker Model Runner, LM Studio, and vLLM, tracking inspection, installed-model inventory, identity mapping, fit, health, endpoint export, benchmarks, and guarded lifecycle as separate capabilities rather than claiming blanket support. Agent planning will add a versioned manifest rendered from existing profile/stack YAML into framework-specific starter configuration while preserving the boundary that Aiplane configures and validates agents but does not execute autonomous workflows. The current scope freeze still blocks implementation of new runtime types until the P0 evidence targets complete.

The remaining evidence/reproducibility gaps are now planned explicitly: versioned YAML/canonical-JSON benchmark suites, repeated stochastic quality summaries, artifact locks, exact external-runner launch manifests, and evidence-backed per-role routing with alternatives and policy constraints. Node REST scheduling and community benchmark exchange are research gates rather than promises; they require user evidence plus authentication, privacy, poisoning/provenance, abuse, versioning, and maintenance decisions. This is planning only and adds no current commands or runtime mutations.
...
- planned post-P0: a versioned placement-evidence schema; quantization/context/KV-cache/runtime-aware resource assessment; repeated throughput, measurable TTFT, latency, and token summaries; comparable local calibration; near-miss explanations; and deterministic task-quality evaluators kept separate from placement/performance metrics;
...
The five feature milestones are implementation-complete. Focused milestone coverage passes 22 tests; affected integration/runtime/provider/stack/CLI regression coverage passes 156 tests plus 13 subtests; the full suite passes 560 tests plus 23 subtests. Repository-wide Ruff format and lint pass. Representative support and adapter CLI checks pass. Docker Model Runner status correctly reports that this host's Docker installation lacks the Docker model command, so cross-platform live evidence remains explicitly open. The full suite also emits the existing structural warning that cli.py is approaching its extraction threshold; that evidence is carried into the next maintainability milestone.
...
2. Next engineering milestone: run a broad maintainability review over implementation and tests. Refactor awkward generated-looking names, comments, control flow, fixtures, and assertions where clearer domain language improves ownership; preserve public contracts, avoid cosmetic churn, and require focused plus full regression evidence.
3. Active evidence follow-up: gather clean-machine Docker Model Runner results on supported Docker Desktop/Engine platforms. This host does not provide the Docker model command and therefore is not cross-platform live evidence.
4. P2 continuation: implement benchmark-suite, artifact-lock, launch-manifest, repeated-quality, and role-routing contracts without growing service scope.
5. Adoption follow-up: observe use of draft imports, support declarations, contributor fixtures, and render-only Kubernetes artifacts before adding any apply service or node scheduling API.
...
| Agents | `agents templates/plan/export` plus `skills/aiplane/SKILL.md` | Initial scaffold / skill guidance implemented; portable manifest hardening planned | Prints non-mutating starter agent application files that use selected model endpoints; the versioned `aiplane` skill is assistant workflow guidance, not a runtime API. A versioned agent-environment manifest rendered from profile/stack YAML is planned to preserve roles, endpoints, policies, approvals, limits, audit labels, and credential references across framework exports without running autonomous workflows. |
...
2. Hard eligibility is evaluated before ranking and reports runtime/API compatibility, policy outcome, memory pool, required memory, available memory, headroom, usable context, and the expected execution/offload mode.
...
3. **Orchestrator and multi-agent workflow metadata** - Implemented / hardening
   ...
   - Define one versioned, reviewable agent-environment manifest contract that can be rendered from profile and stack YAML. It should preserve model aliases, provider/runtime endpoints, role bindings, tool policy, approval mode, limits, audit labels, and credential references, then compile into framework-specific starter configuration without embedding secrets or claiming to run the agents.
...
5. **Runtime, stack lifecycle, and endpoint hardening** - Implemented foundation / ongoing lifecycle hardening
   ...
   - Prioritize Docker Model Runner parity first: close its detection, installed-model inventory, identity mapping, fit, health, endpoint export, benchmark, and guarded lifecycle gaps through official Docker Desktop contracts before pursuing broader runner-parity expansion.
   - Maintain a capability matrix for Ollama, llama.cpp, MLX, Docker Model Runner, LM Studio, and vLLM. Track separately: platform detection, installed-model inventory, alias/native-ID resolution, model-fit assessment, health/readiness, endpoint export, benchmark support, and guarded install/pull/start/stop operations.
   ...
   - Add an artifact lock and external-runner launch manifest shared by pull, stack plan/doctor, benchmark, replay, and lifecycle helpers. Keep vendor commands exact and previewable; do not reinterpret a launch plan as permission to install a runtime, bind a public port, or copy model weights.
...
6. **Cloud, VM, and workstation workflow hardening** - Implemented foundation / in progress
   ...
   - After Docker Model Runner parity, add a render-only Kubernetes artifact family for `ResourceClaim`, `Deployment`, `Service`, and Helm values. Rendering must be deterministic, secret-free, reviewable, and non-mutating; applying generated artifacts remains a separate future capability behind explicit human review and approval.
...
8. **Runtime packaging and deployment reproducibility** - Planned
   - Deliver runner coverage in staged tiers: Docker Model Runner parity first through its official Docker Desktop contracts; then Ollama and vLLM hardening; llama.cpp and LM Studio first-class inventory/endpoint/benchmark coverage; and MLX support on Apple Silicon.
...
10. **Benchmark and recommendation quality** - Planned
   - Add a versioned placement-evidence schema that keeps hard compatibility, resource estimates, measured performance, task-quality evidence, user configuration, and policy decisions distinct.
   - Resolve model variants using artifact format, quantization, total/active parameters, context and KV-cache assumptions before producing memory and execution-mode findings.
   - Add repeated benchmark runs with throughput, measurable time-to-first-token, end-to-end latency, prompt/output token counts, runtime/model settings, environment fingerprints, robust summaries, and comparability metadata.
   ...
   - Define portable, versioned YAML plus canonical-JSON benchmark suites with safe grader allowlists, deterministic validation, explicit prompts/fixtures/settings/budgets, and no implicit arbitrary-code execution.
   - Repeat stochastic quality evaluations with recorded decoding settings and robust pass-rate/score uncertainty; keep deterministic exact/schema/tool-call checks distinct from sampled judge- or rubric-based evidence.
   - Produce evidence-backed per-role routing comparisons that retain alternatives and policy/endpoint constraints instead of turning role choice into one opaque universal score.
...
- Versioned benchmark-suite schema, repeated stochastic quality summaries, artifact locks, and exact external-runner launch manifests.
