...
The current uncommitted follow-up adds `agents doctor`: it observes installed distribution metadata without imports and supports an opt-in, credential-free endpoint inventory probe for direct or stack-derived roles. It does not establish framework-version semantic compatibility; that remains an evidence task requiring externally installed framework and endpoint validation, still opt-in, secret-free, and outside Aiplane's agent-execution boundary. The manual validation guide now starts with a concise grouped A–F path for installation/profile isolation, host and catalog discovery, local runtime/chat configuration, host-client exports, agent-environment evidence, and reproducibility/stacks, before the exhaustive acceptance checklist. Validation for the current diff: focused security-documentation, agent, and boundary tests pass 38 tests; the full suite passes 828 tests with 9 opt-in checks skipped; formatting, lint, and diff hygiene pass.
...
| Agents | `agents templates/plan/manifest/doctor/export/job/handoff` plus `skills/aiplane/SKILL.md` | Versioned handoff, enforcement-status, and starter-readiness guidance implemented | Prints non-mutating starter files, a schema-linked agent-environment manifest, stack-bound job specifications, and checksummed environment/job handoffs for seven supported targets. Every target includes a direct OpenAI-compatible endpoint-smoke starter with a minimal independent requirement file; native `agent.py` remains limited to LangGraph and the simple OpenAI client. `agents doctor` observes installed distribution metadata without imports and optionally performs a credential-free `GET /models` probe for every selected role; it never starts agents, sends prompts, or reads credentials. Readiness validates Python, endpoint/protocol, and declared chat role and reports observed framework package versions without inferring compatibility from them. `control_enforcement` explicitly separates Aiplane validation/recording from runtime enforcement for workspace, tools, approvals, limits, and audit labels. It preserves multi-role identities, endpoints, policy, approvals, limits, audit labels, credential references, task text, and workspace boundaries without secrets, package installation, job queuing, or agent execution. |
...
6. **Agent-to-agent setup and export - Implemented configuration foundation / live validation ongoing**
   ...
   - Export starter configuration for established orchestrator frameworks, including an endpoint smoke path for each target, observed local package-version readiness, and opt-in credential-free endpoint inventory probes. Live framework-version semantic compatibility validation remains ongoing, and autonomous agent execution remains outside `aiplane`.
