...
| Agents | `agents templates/plan/manifest/export/job/handoff` plus `skills/aiplane/SKILL.md` | Versioned environment/job handoff and framework starter guidance implemented | Prints non-mutating starter files, a schema-linked agent-environment manifest, stack-bound job specifications, and checksummed environment/job handoffs for seven supported targets. It preserves multi-role identities, endpoints, policy, approvals, limits, audit labels, credential references, task text, and workspace boundaries without secrets, package installation, job queuing, or agent execution. |
