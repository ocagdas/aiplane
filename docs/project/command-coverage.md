...
| Models | `models list/show/defaults/use/enable/disable/refresh/promote/clear-cache/pull/test/benchmark` | Implemented / ongoing | `refresh` and `promote` include next-step guidance for the reviewed path from provider discovery to generated aliases to curated profile aliases. |
